...
export function shuffle(array: Friend[]): Friend[] {
...
export function formatDate(date: Date, format?: string): string {
    ...
    const separator = ['.', '-', '/'].includes(configSeparator.trim()) ? configSeparator.trim() : '.'
    ...
    switch (formatStr) {
        case 'YYYY-MM-DD': return `${year}${separator}${pad(month)}${separator}${pad(day)}`
        default: return `${year}${separator}${pad(month)}${separator}${pad(day)}`
    }
}
